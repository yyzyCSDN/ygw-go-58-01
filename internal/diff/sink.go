...
func (s *Sink) WriteResult(ctx context.Context, window model.Window, result *model.Result, resume ResumeInfo) error {
	...
	for _, entry := range result.Entries {
		if err := s.writeEntry(ctx, entry, resume); err != nil {
			if firstErr == nil {
				firstErr = fmt.Errorf("write result for %s: %w", window.ID, err)
			}
		}
	}
	return firstErr
...
// writeEntry 按去重规则写入单条差异。
func (s *Sink) writeEntry(ctx context.Context, entry model.Entry, resume ResumeInfo) error {
	if resume.CommittedKey != "" && entry.Key <= resume.CommittedKey {
		return nil
	}
	key := entry.DedupKey()
	if s.dedup.Seen(key) {
	...
	if err != nil {
		return fmt.Errorf("check dedup key %s: %w", key, err)
	}
	if exists {
		s.dedup.Mark(key)
		return nil
